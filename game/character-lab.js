...
const movement=createMovement(T,{sampleGround:()=>0,stickElement:$('stick'),jumpButton:$('jump'),runButton:$('run')});
...
function reset(){time=0;movement.reset();Object.assign(preview,{speed:0,yaw:0,grounded:true,verticalVelocity:0});hero.position.set(0,0,0);hero.rotation.y=0;animator.reset();animator=createAnimator(T,hero,clip==='free'?movement:preview);}
...
function advance(dt){
 ...
 if(clip==='free'){movement.update(dt);hero.position.copy(movement.position);hero.rotation.y=movement.yaw;}
  ...
  preview.speed=clip==='walk'?2.2:clip==='run'||clip==='turn'?4.5:0;
...
function render(){
 ...
 window.__LAB__={clip,time,frozen,grounded:clip==='free'?movement.grounded:preview.grounded,position:hero.position.toArray(),draws:renderer.info.render.calls};
}
