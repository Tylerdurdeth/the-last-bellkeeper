// Atlas woodland-r09: b/shelf-stump. Independent construction candidate.
export default function generate(T) {
  ...
  // B: hollow jagged lathed trunk, swept root flares and concentric shelf profiles.
  const bark=material(0x493b32,'timber'),cut=material(0x896a48,'timber'),moss=material(0x687843),amber=material(0xb87634),rim=material(0xe0b06c),band=material(0xca9146);
  const trunk=lathe([[.28,0],[.20,.12],[.145,.55],[.17,.86],[.105,.86],[.085,.22],[0,.19]],bark,[0,0,0]);const p=trunk.geometry.attributes.position;for(let i=0;i<p.count;i++){const x=p.getX(i),y=p.getY(i),z=p.getZ(i),a=Math.atan2(x,z);p.setY(i,y+(y>.6?.11*Math.sin(a*3)+.045*Math.cos(a*5):0));}trunk.geometry.computeVertexNormals();
  for(let i=0;i<5;i++){const a=i*1.257;sweep([[Math.cos(a)*.12,.3,Math.sin(a)*.12],[Math.cos(a)*.2,.1,Math.sin(a)*.2],[Math.cos(a)*.33,.015,Math.sin(a)*.33]],.045,bark);}
  for(let i=0;i<8;i++){const a=i*2.399,r=.16,y=.15+i*.075,g=joint('shelf'+i,[Math.sin(a)*r,y,Math.cos(a)*r]);g.rotation.y=a;const s=lathe([[0,0],[.10,-.015],[.20,0],[.21,.014],[.17,.05],[.07,.065],[0,.055]],amber,[0,0,.075],[1,1,.7],g);lathe([[.19,.027],[.21,.014],[.197,.008]],rim,[0,0,.075],[1,1,.7],g);lathe([[.13,.056],[.15,.047],[.165,.043]],band,[0,0,.075],[1,1,.7],g);}
  for(let k=0;k<6;k++){const a=k*Math.PI/3;rod([Math.sin(a)*.195,.1,Math.cos(a)*.195],[Math.sin(a)*.147,.68,Math.cos(a)*.147],.013,cut,root,.008);}
  for(let i=0;i<15;i++){const a=i*2.399;low(moss,[Math.sin(a)*.155,.055+i*.045,Math.cos(a)*.155],[.07,.045,.06]);}
  return finish('shelf-stump',.9);
}
