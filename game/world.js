...
export async function buildWorld(scene,art){
 ...
 const cottage=place('cottage',-8,9,1,.7,{dynamic:true});colliders.push({x:-8,z:9,r:2.25});const cottageMats=[];cottage.traverse(o=>{if(o.isMesh){o.material=o.material.clone();art.style(o);cottageMats.push(o.material);}});
 ...
 for(const [cx,cz] of [[-6,15],[-2,11],[-10,12],[-12,3],[-18,-3],[-14,-9],[-12,-17],[-7,-18],[0,6],[10,-10],[-2,21],[4,19],[-7,20],[2,16]])for(let i=0;i<16;i++){const angle=rnd()*6.28,r=.4+rnd()*2.2,x=cx+Math.cos(angle)*r,z=cz+Math.sin(angle)*r;if(pathDistance(x,z)<1.65||Math.hypot(x+8,z-9)<2.7||Math.hypot(x+10,z+17)<1.8||Math.hypot(x-8,z+10)<2.3||Math.hypot(x+8,z+15)<2)continue;place(i%4?'fern':'flower',x,z,.8+rnd()*.8,angle);}
 ...
 function ground(x,z){const rockY=rockGround(x,z);if(rockY!==null)return Math.max(height(x,z),rockY);if(Math.abs(x)>35||Math.abs(z)>35)return null;if(restored&&Math.abs(x-8)<1.1&&z> -6.2&&z<.2)return 1.2-.15*Math.cos(Math.min(1,Math.abs(z+3.35)/2.5)*Math.PI/2);const h=height(x,z);return h< -2?null:h;}
 function blocked(x,z,r){return colliders.some(c=>Math.hypot(x-c.x,z-c.z)<c.r+r);}
 function update(dt,t,pos,isRestored,gentle,camera){
  ...
  const cdx=cottage.position.x-pos.x,cdz=cottage.position.z-pos.z,front=cdx*.615+cdz*.788,side=Math.abs(cdx*.788-cdz*.615);cottageOpacity=T.MathUtils.damp(cottageOpacity,front> -1&&front<12&&side<3.4?.16:1,12,dt);for(const m of cottageMats){m.transparent=cottageOpacity<.99;m.opacity=cottageOpacity;m.depthWrite=cottageOpacity>.98;}
  ...
  occlusionTimer-=dt;if(camera&&occlusionTimer<=0){occlusionTimer=.12;for(const tree of trees){tree.userData.occluded=(tree.userData.occludedUntil||0)>t;if(tree.position.distanceTo(pos)>21)continue;for(const h of [.45,1.25]){const target=new T.Vector3(pos.x,pos.y+h,pos.z),dir=target.sub(camera.position),dist=dir.length();cameraRay.set(camera.position,dir.normalize());cameraRay.far=dist-.15;if(cameraRay.intersectObject(tree,true).length){tree.userData.occluded=true;tree.userData.occludedUntil=t+.65;break;}}}}
  for(const tree of trees){tree.userData.fade=T.MathUtils.damp(tree.userData.fade,tree.userData.occluded?0:1,18,dt);tree.visible=tree.position.distanceTo(pos)<39&&tree.userData.fade>.012;for(const m of tree.userData.fadeMats){m.opacity=tree.userData.fade;m.transparent=m.opacity<.99;m.depthWrite=m.opacity>=.99;}}
 ...
 return {ground,blocked,keepsakePoint,update,bridge,wheel,chimes,landmarks:POINTS};
}
