...
const out='evidence/rebuild/discovery-review/pocket-retest';await fs.mkdir(out,{recursive:true});
...
for(const phone of [false,true]){
 ...
 await action('chime','chime');for(const [x,z]of [[-9,-19.5],[-7,-19.5],[-7,-18.7]])await go(x,z,'quiet');await sleep(300);await shot(prefix+'-quiet');
 for(const [x,z]of [[-9,-19.5],[-11,-19.5],[-13.2,-17.5]])await go(x,z,'responsive');await sleep(300);await shot(prefix+'-responsive');await action('capture','capture');await shot(prefix+'-captured');
}
result.pass=true;
