...
export function createArtDirection(T, renderer) {
 ...
 diffuseColor.rgb*=mix(vec3(1.),.42+1.6*bkStone,.85);
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        vec3 bkPaintedLight = mix(vec3(.48, .65, .70), vec3(.88, .88, .72), smoothstep(.02,.48,bkDirect));
        bkPaintedLight = mix(bkPaintedLight, vec3(1.18, 1.08, .85), smoothstep(.45,.95,bkDirect));
    ...
    m.customProgramCacheKey = () => `bellkeeper-material-study-C:${key}:${metal ? 1 : 0}:${foliage ? 1 : 0}:${priorKey}`;
    return m;
  }
