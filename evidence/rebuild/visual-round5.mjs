...
const out='evidence/rebuild/visual-round5';await fs.mkdir(out,{recursive:true});
...
for(const phone of [false,true]){
 ...
 for(const [x,z]of [[-5,13],[-4,8],[-10,5]])await go(x,z,'roots');await shot(prefix+'-roots');
 for(const [x,z]of [[-15,0],[-16,-7],[-10,-10]])await go(x,z,'garden-approach');
 ...
 await go(-8,-15,'chime');await shot(prefix+'-chime-arrival');await action('chime',prefix+'-ring');
 for(const [x,z,label]of [[-7.3,-14.4,'east'],[-8.5,-13.5,'south'],[-10,-15,'west']]){await go(x,z,label);await shot(prefix+'-'+label);}
 await go(-10,-17,'capture');await action('capture',prefix+'-capture');await shot(prefix+'-capture');
 await go(-10,-10,'retreat');await rec.stop();await fs.writeFile(out+'/'+prefix+'-transition.webm',Buffer.concat(chunks));
}
result.pass=true;
