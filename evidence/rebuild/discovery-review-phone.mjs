...
const out='evidence/rebuild/discovery-review/phone';await fs.mkdir(out,{recursive:true});
...
try{
 ...
 await action('chime','awakened');for(const [x,z,label]of [[-9,-19.5,'quiet-approach'],[-7,-19.5,'quiet-near'],[-7,-18.7,'quiet-bed']])await go(x,z,label);result.checks.quietPocket={pass:!(await read()).charged&&(await read()).context!=='capture'&&(await read()).quietObserved};await shot('quiet-bed');await sleep(450);await shot('quiet-later');for(const [x,z,label]of [[-9,-19.5,'current-trail'],[-11,-19.5,'current-approach'],[-13.2,-17.5,'garden']])await go(x,z,label);await shot('responsive-before');await sleep(450);await shot('responsive-later');await action('capture','captured');
 ...
 for(const [x,z,label]of [[-3,2,'shortcut-return'],[0,6,'wheel-approach'],[4,4,'wheel-path'],[5,3.7,'wheel']])await go(x,z,label);
 ...
 await tap('#pause');await sleep(100);await page.tap('#reset');await sleep(200);const reset=await read();result.checks.reset={pass:!reset.porchRead&&!reset.quietObserved&&reset.keepsakes===0&&reset.score===0&&!reset.charged&&!reset.awakened&&!reset.restored,state:reset};await shot('reset');result.pass=Object.values(result.checks).filter(c=>'pass'in c).every(c=>c.pass)&&!result.errors.length&&!result.requests.length;
