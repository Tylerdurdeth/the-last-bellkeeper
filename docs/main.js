...
const state={started:false,paused:false,t:0,charged:false,awakened:false,restored:false,complete:false,keepsakes:new Set(),action:null,actionTime:0,muted:localStorage.getItem('bellkeeper-muted')==='1',gentle:localStorage.getItem('bellkeeper-gentle')==='1'};
let hero,movement,animator,world,staff,staffHand,captionEnd=0,context=null,lastFoot=0,travel=0,visited=new Set(),fps=60,audio;
...
const source=point(POINTS.garden,.55),wheelPoint=point(POINTS.wheel),chimePoint=point(POINTS.chime),finish=point(POINTS.overlook);
const memories=[{p:point([-19,-5]),text:'A copper feather. Someone once kept the birds company.'},{p:point([-4,-20]),text:'A scratched bell: “For the mornings we haven’t met yet.”'},{p:point([11,-11]),text:'A ribbon tied toward home. The wind remembers the way.'}];
...
function updateUI(){
 ...
 $('#chargeText').textContent=(state.charged?'Wind held in the bell':'Bell empty')+' · '+state.keepsakes.size+'/3 keepsakes';$('#chargeIcon').textContent=state.charged?'✧':'◌';
...
function action(){if(!state.started||state.paused||state.action)return;if(!context){caption('Listen. Watch the leaves. There is still a little wind here.',3);return;}
 if(context.kind==='chime'){beginAction('release',()=>{state.awakened=true;caption('A note stirs the still leaves. The little current is awake.',4);});}
 if(context.kind==='capture')beginAction('capture',()=>{state.charged=true;caption('It’s warm in your hand. The old wheel may remember this song.',4);});
 ...
 if(context.kind==='finish'){state.complete=true;caption('One bell answers another. Stay a while—there are still small things to find.',7);sound('restore');}
 updateUI();}
...
$('#reset').onclick=()=>{Object.assign(state,{charged:false,awakened:false,restored:false,complete:false,action:null,actionCallback:null});state.keepsakes.clear();visited.clear();movement.reset(START);animator.reset();travel=0;pause(false);updateUI();caption('Another morning. The woods are listening.',4);};
...
function findContext(){const p=movement.position;context=null;
 for(let i=0;i<memories.length;i++)if(!state.keepsakes.has(i)&&p.distanceTo(memories[i].p)<(i===0?.72:1.4)&&Math.abs(p.y-memories[i].p.y)<(i===0?.25:2))context={kind:'memory',index:i,label:'Keep the memory'};
 ...
 const areas=[['cottage',point(POINTS.cottage),5,'The keeper’s cottage. Fresh ribbons, but nobody at the door.'],['crossing',wheelPoint,4,'The far bank is out of reach. A path curls back into the roots.'],['garden',source,6,'You hear a faint chime beneath the leaves.']];for(const [id,pnt,d,text] of areas)if(!visited.has(id)&&p.distanceTo(pnt)<d){visited.add(id);caption(text,4);}
}
...
function frame(now){requestAnimationFrame(frame);const elapsed=(now-last)/1000;last=now;const dt=Math.min(.05,elapsed);fps=T.MathUtils.lerp(fps,1/Math.max(.001,elapsed),.03);
 ...
 animator.update(dt,{time:state.t,action:state.action,actionProgress:state.actionTime/.68,charged:state.charged});poseStaff();world.update(dt,state.t,p,state.restored,state.gentle,camera);soundscape.update(dt,{position:p,speed:movement.speed,grounded:movement.grounded,charged:state.charged,restored:state.restored,gardenDistance:p.distanceTo(source),waterDistance:Math.abs(p.z+3.35)});
 ...
 for(let i=0;i<memoryGlows.length;i++){memoryGlows[i].visible=!state.keepsakes.has(i);memoryGlows[i].rotation.y+=dt;memoryGlows[i].position.y=memories[i].p.y+.4+Math.sin(state.t*2+i)*.08;}
 ...
 renderer.render(scene,camera);window.__READY__=true;window.__GAME__={pos:[pos.x,pos.z],y:pos.y,fps,speed:movement.speed,mode:movement.mode,grounded:movement.grounded,score:state.complete?1:0,draws:renderer.info.render.calls,tris:renderer.info.render.triangles,charged:state.charged,awakened:state.awakened,restored:state.restored,paused:state.paused,started:state.started,keepsakes:state.keepsakes.size,travel,context:context?.kind||null};
 }else renderer.render(scene,camera);
}
try{await art.ready;world=await buildWorld(scene,art);memories[0].p.copy(world.keepsakePoint);hero=await ASSET('./assets/hero.js',{keepHierarchy:true});art.style(hero,{character:true});scene.add(hero);staff=await ASSET('./assets/staff.js');art.style(staff);staff.scale.setScalar(.83);staffHand=hero.userData.joints.rightHand;staffHand.add(staff);
