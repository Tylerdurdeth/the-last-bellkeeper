...
function updateUI(){ $('#chargeText').textContent=state.pendingRestore?'Current released':state.charged?'Wind in the bell':'Bell empty';$('#chargeIcon').textContent=state.charged?'✧':'◌';$('#action').innerHTML=(state.charged?'Release wind':'Catch wind')+' <span>SPACE</span>';$('#objective').textContent=state.restored?'The terrace is breathing again':state.pendingRestore?'Follow the returning current':state.charged?'Carry the gust to the seed wheel':'Find the wandering gust';$('#sound').textContent='Sound: '+(state.muted?'off':'on');$('#motion').textContent='Gentle motion: '+(state.gentle?'on':'off');}
...
let last=performance.now(),fps=60;function frame(now){requestAnimationFrame(frame);const elapsed=(now-last)/1000;last=now;fps=T.MathUtils.lerp(fps,1/Math.max(.001,elapsed),.04);const dt=Math.min(elapsed,.04);
 ...
 for(const ob of [{x:3,z:-1.2,r:.85}]){const dx=pos.x-ob.x,dz=pos.z-ob.z,d=Math.hypot(dx,dz);if(d<ob.r){pos.x=ob.x+dx/Math.max(.001,d)*ob.r;pos.z=ob.z+dz/Math.max(.001,d)*ob.r;}}
 const gx=(pos.x+3.35)/2.65,gz=(pos.z+2.45)/2.1,gd=Math.hypot(gx,gz);if(gd<1){pos.x=-3.35+gx/Math.max(.001,gd)*2.65;pos.z=-2.45+gz/Math.max(.001,gd)*2.1;}
 ...
 const markerPoint=(state.charged||state.pendingRestore?target:source).clone();markerPoint.y+=1.2;markerPoint.project(camera);const mx=(markerPoint.x*.5+.5)*innerWidth,my=(-markerPoint.y*.5+.5)*innerHeight;const marker=$('#targetMarker');marker.hidden=!state.started||state.paused||state.restored;marker.style.left=Math.max(75,Math.min(innerWidth-75,mx))+'px';marker.style.top=Math.max(110,Math.min(innerHeight-280,my))+'px';marker.textContent=state.charged||state.pendingRestore?'✧  Seed wheel':'◌  Wandering gust';
...
try {
 ...
 for(const [x,z,s] of [[-25,-35,3.5],[0,-45,3.6],[26,-35,3.5]]){const t=await asset('tree');t.position.set(x,-18,z);t.scale.setScalar(s);scene.add(t);}
