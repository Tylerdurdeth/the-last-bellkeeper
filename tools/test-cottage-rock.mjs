...
await move(['KeyW'],2000);await move(['KeyW'],2500);await page.keyboard.press('Space');await move(['KeyD'],1800);result.states.approach=await move(['KeyW'],3000);await page.screenshot({path:out+'/approach.png'});
result.states.run=await move(['KeyW','ShiftLeft'],3500);await page.screenshot({path:out+'/run.png'});
result.states.jump=await move(['KeyW','KeyJ'],1300);await page.screenshot({path:out+'/jump.png'});
