...
const b=await puppeteer.launch({headless:true});const p=await b.newPage();const errors=[];p.on('pageerror',e=>errors.push(e.message));const sleep=ms=>new Promise(r=>setTimeout(r,ms));
try{
 ...
 await p.select('#clip','idle');await sleep(100);await p.screenshot({path:'evidence/character-lab/phone.png'});await p.select('#view','face');await sleep(100);await p.screenshot({path:'evidence/character-lab/face.png'});assert.deepEqual(errors,[]);console.log('PASS modes, freeze, frame step, keyboard movement, real emulated touch, desktop and phone rendering');
