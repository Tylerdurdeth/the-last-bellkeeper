...
const b=await puppeteer.launch({headless:true});const p=await b.newPage();const errors=[],meshRequests=[];p.on('request',r=>{if(/\.(glb|gltf|fbx|obj)(\?|$)/i.test(r.url()))meshRequests.push(r.url());});p.on('pageerror',e=>errors.push(e.message));const sleep=ms=>new Promise(r=>setTimeout(r,ms));
try{
