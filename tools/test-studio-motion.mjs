...
await p.select('#clip','free');await p.keyboard.down('ShiftLeft');await p.keyboard.down('KeyW');await wait(90);const start=await sample('start');assert(start.speed>0&&start.speed<6.9);
await wait(600);assert((await state()).speed>6.8);await p.keyboard.up('KeyW');await p.keyboard.down('KeyD');await wait(100);const turn=await sample('turn');assert(Math.abs(turn.motion.bank)>.015);
await wait(200);await p.keyboard.up('KeyD');await wait(50);const stop=await sample('brake');assert(stop.speed>0&&stop.speed<6.8);await p.keyboard.up('ShiftLeft');await wait(650);assert((await state()).speed<.02);assert.equal((await state()).authoredClip,'Idle_Loop');
for(let attempt=0;attempt<2;attempt++){await p.keyboard.press('KeyJ');await wait(70);assert(!(await state()).grounded);await sample(`jump-${attempt}`);let landed=false;for(let i=0;i<30;i++){await wait(25);const s=await state();if(s.grounded){assert.equal(s.authoredClip,'Jump_Land');landed=true;break;}}assert(landed);await wait(100);await sample(`landing-${attempt}`);assert.equal((await state()).authoredClip,'Jump_Land');await wait(600);assert.equal((await state()).authoredClip,'Idle_Loop');}
