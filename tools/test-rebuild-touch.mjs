...
const out='evidence/rebuild/touch-round2';await fs.mkdir(out,{recursive:true});
...
try{
 ...
 await action('chime','awakened');await go(-10,-17,'garden');await action('capture','captured');
 for(const [x,z,label]of [[-9,-19.5,'hidden-garden-edge'],[-7,-19.5,'hidden-garden'],[-4,-20,'second-keepsake']])await go(x,z,label);await action('memory','keepsake-garden');
 for(const [x,z,label]of [[-7,-19.5,'garden-return'],[-9,-19.5,'garden-edge-return'],[-10,-17,'garden-again'],[-10,-10,'return-upper'],[-16,-7,'return-bend'],[-15,0,'return-west'],[-10,5,'return-roots'],[-4,8,'return-cottage'],[0,6,'wheel-approach'],[4,4,'wheel-path'],[5,3.7,'wheel']])await go(x,z,label);
 ...
 await tap('#pause');await sleep(100);await page.tap('#reset');await sleep(200);const reset=await read();result.checks.reset={pass:reset.keepsakes===0&&reset.score===0&&!reset.charged&&!reset.awakened&&!reset.restored,state:reset};await shot('reset');result.pass=result.checks.fullTouch.pass&&result.checks.reset.pass&&!result.errors.length&&!result.requests.length;
