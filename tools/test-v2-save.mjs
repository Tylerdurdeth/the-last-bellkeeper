...
await go(P.maraOutlet.x, P.maraOutlet.z); await page.waitForFunction(() => window.__GAME__.contextId === 'mara'); await page.keyboard.press('Space');
await page.waitForFunction(() => window.__GAME__.charged, { timeout: 3000 });
...
assert(after.quest.staff && after.quest.bypass && after.charged, 'progress and held gust restored');
assert(Math.hypot(after.pos[0] - before.pos[0], after.pos[1] - before.pos[1]) < 1.5, 'resumes at the safe checkpoint');
assert.equal(after.objective, 'Give the gust to the seed wheel'); assert(!after.introActive);
assert.deepEqual(errors, []);
await browser.close();
console.log('PASS: v1 save ignored with a friendly note; v2 Continue restores progress, held gust and checkpoint');
